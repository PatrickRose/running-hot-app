...
**Every Facility gets a private text and voice channel**, which is where its Runs will happen. They sit in a category of their own per Corporation (`<Corp> Facilities`) rather than beside its team channels, so eight Facilities do not bury the two channels its players actually talk in — Discord allows 50 channels per category, which is 24 Facilities at two each. Locked to Control and the owning Corporation; the Runners attacking a Facility are added when a Run starts, because they choose their target in Secret (3.4.1) and access any earlier would leak who is hitting what.
