...
- A Facility build cost comes from its type's own price on the type sheet, less the Corporation's `facility_build_discount` — MCM's Construction Leader, seeded at 2 and nought for everybody else — and Control can name another. What is *not* derived is Income: more Corporate Facilities is a *reason* for Control to raise it, never a formula that raises it.
...
**A CEO builds their own Facilities; Control keeps the overrides.** Every Corporate seat reads the type sheet on `/facilities` — prices, what each type grants, and what a Runner gets for accessing one — because it is the whole Corporation's business, and before it was there the only way to learn a price was to ask Control. CEOs are the only ones allowed to build Facilities (`CorporationPolicy::requisition`) — a ruling, not a sign-off step: there is no requisition slip for Security to raise — and the player route pays the type sheet's price with no cost field at all: it opens next turn, during Setup only, and the refusals are `RequisitionFacility`'s, shared with Control's panel. Control's form keeps both overrides — a blank cost is what the Corporation would pay itself, 0 is free, any other number is a ruling — and Build now.

**Players see only the types they can build.** `App\Support\BuildableFacilityTypes` is the list, and it is both what `/facilities` draws and what the CEO's route refuses against: the types marked `available_from_start` (Research, Security and Corporate — the three 3.3.1 names), plus any type a technology the Corporation holds *and can use* prints "Unlock: <type> facility" for. Usable is `TechnologyService::isUsable()`, so a claimed copy unlocks nothing and a thief needs every piece of a split technology. The match is read off the effect text by `FacilityType::isUnlockedBy()`, the way a split technology is read off its name, so a technology Control writes mid-game unlocks a type by saying so; a trailing "Facility" on the type's own name is ignored so "ID facility" finds "ID Facility". A type Control invents starts closed and is on nobody's sheet until Control flips it to "Everyone" in the type catalogue or writes a technology that unlocks it — a new type is researched (footnote 9), so announcing it to every Corporation would be the leak. Control builds any type for anybody from its own panel regardless. `FacilityRequisitionTest` checks every "Unlock: … facility" in the technology list names a real type, and every closed type is unlocked by something, because a typo there hides a type silently.

**Construction Leader is a number on the Corporation, not a reading of the technology.** Its printed effect is 2 Credits off every Facility built, and in practice only MCM ever holds it, so `corporations.facility_build_discount` is seeded 2 for MCM from `config/running_hot.php` (and by its migration for games already running) and `Corporation::facilityBuildCost()` is the one place it is subtracted, never below nought. Deriving it from `technology_holdings` would mean a Run that destroys the card or a thief holding both pieces changes the price on its own, which is exactly the sort of consequence this application leaves to Control — who edits the discount on the Facilities panel. The card's second clause, building for other Corporations, is not modelled. The name clash check lives in the action for the same reason.
