...
**Who is chairing is said outright on Control's panel**, not left to be found. It was only a badge inside the rotation list, and a seat can be in the Chair without being in the rotation at all — which is how every game opens, with HM Government chairing and the panel saying so nowhere, since the Government sits in the "seats that never come round to chair" line underneath. The sitting now names the Chair at the top of its own card, and both lists mark the chairing seat. The Chamber already said it in as many words, which is why nothing moved there.

**The rotation reads as columns, not as a wrapped row of controls.** Every row is the same shape — number, badge, name, the two arrows, then the actions — with the name taking the slack, so everything after it starts at the same place down the list. Anything that varies per row (the In the Chair badge, a seat's Remove) sits *inside* a column rather than between two of them, which is what made the list ragged when it was one `flex-wrap` line. The buttons lost their sentences for the same reason: five copies of "Give them the Chair now" is what the width was going on, and the sentence lives in each button's `aria-label` where it is still read out.
